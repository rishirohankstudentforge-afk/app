...
async function main() {
  // 1. Delete the incorrect Easy Sprint
  await prisma.hackathon.deleteMany({
    where: { title: "Redlix Easy Sprint" }
  });

  // 2. Find the MRDU Hackathon to link to
  const mrduHackathon = await prisma.hackathon.findFirst({
    where: { title: "HakITxMRDU" }
  });
  ...
  for (let i=1; i<=15; i++) {
    ...
    let expected = [];
    for (let j=1; j<=n; j++) {
      if (j%3===0 && j%5===0) expected.push("FizzBuzz");
      else if (j%3===0) expected.push("Fizz");
  ...
  const questions = [
    ...
    },
    {
      ...
      type: "coding",
      difficulty: "Easy",
      description: "Given an integer n, return a string array where answer[i] == 'FizzBuzz' if i is divisible by 3 and 5, 'Fizz' if divisible by 3, 'Buzz' if divisible by 5, and i (as a string) otherwise. Note: output should be exactly formatted as an array of strings.",
      timeLimit: 15,
      testCases: fizzBuzzTC
    },
    {
      title: "9. Second Highest Salary (SQL)",
      type: "sql",
      difficulty: "Easy",
  ...
  const sprintId = `ESY-${Math.floor(100000 + Math.random() * 900000)}`;

  const sprint = await prisma.hackathon.create({
    data: {
      title: "Redlix Easy Sprint",
      description: "A 2hr 30min sprint packed with 8 coding and 1 SQL questions, rigorously tested with 15 test cases each.",
      startDate: new Date(),
  ...
  console.log("Created Correct Easy Sprint:", sprint.title);
